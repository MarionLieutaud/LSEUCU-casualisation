...
# theme for patchwork plots
personal_theme_simple_patchwork <- 
  theme_ipsum(base_size=9) +
...
```
```{r patchworkHESA}
# prep for patchwork plot later
y1_hesa <- h_lse_long$value[h_lse_long$contract.type == "fixed-term" & h_lse_long$date == "2021-2022"]
y2_hesa <- h_lse_long$value[h_lse_long$contract.type == "permanent" & h_lse_long$date == "2021-2022"]
y_mid_hesa <- (y1_hesa+y2_hesa)/2
label_hesa <- paste0("<span style='color:red'>**", 
                round(h_lse_long$latest_prop_casual_2022[h_lse_long$date == "2021-2022" & h_lse_long$contract.type == "fixed-term"], digits=1), "%",
                "**</span> <br> casualised <br> academic staff <br> in 2021-2022")

p4_patchwork <- p4 +
  geom_segment(aes(x = "2021-2022", xend = "2021-2022", y=y1_hesa, yend = y2_hesa), linetype = 2, colour = 'grey') +
  geom_richtext(aes(x = "2021-2022", y = y_mid_hesa, label = label_hesa), 
                hjust = 1, size = 4, fill = NA, label.colour = NA, family = "Arial Narrow") +
  scale_y_continuous(limits = c(690, 1080)) +
  labs(title = "HESA data") +
  personal_theme_simple_patchwork +
...
hr <- hr %>%
  mutate(academic_year = str_remove(academic_year, " Staff record"), # keep only dates
         ...
         staff_category = case_when(
           ...
           Staff.Type %in% c("Guest Teacher", "LSE Fellow", "Policy", "Research", "Teaching", "Teaching and Research") ~ "Academic positions"),
...
tab <- hr %>%
  ...
  mutate(academic_employment = recode(academic_employment, 
                                      "Academic (fixed-term)" = "fixed-term",
                                      "Academic (permanent)" = "permanent")) %>%
...
tab <- tab %>%
  ...
  mutate(total_academic = sum(sum),
         total_casual_academic = sum[academic_employment == "fixed-term"],
...
p_hr <- tab %>% ggplot(aes(x=academic_year, y=sum, group=academic_employment)) +
...
#ggsave(p_hr_standalone, filename = "plot_LSEHR_nbstaff.png", width = 6.5, height = 4) # save plot
```
\newpage

```{r patchworkHRprep}
# prep for patchwork
y1_hr <- tab$sum[tab$academic_employment == "fixed-term" & tab$academic_year == "2021-2022"]
y2_hr <- tab$sum[tab$academic_employment == "permanent" & tab$academic_year == "2021-2022"]
y_mid_hr <- (y1_hr+y2_hr)/2
label_hr <- paste0("<span style='color:red'>**", 
                round(tab$prop_casual[tab$academic_year == "2021-2022" & tab$academic_employment == "fixed-term"], digits=1), "%",
                "**</span> <br> casualised <br> academic staff <br> in 2021-2022")

p_hr_patchwork <- p_hr +
  geom_segment(aes(x = "2021-2022", xend = "2021-2022", y=y1_hr, yend = y2_hr), linetype = 2, colour = 'grey') +
  geom_richtext(aes(x = "2021-2022", y = y_mid_hr, label = label_hr), 
                hjust = 1, size = 4, fill = NA, label.colour = NA, family = "Arial Narrow") +
  geom_richtext(aes(x = "2015-2016", y = y_mid_hr-50, label = "no LSE <br> HR data <br> before <br> 2016-2017"), 
                hjust=0.5, size = 3, fill = NA, label.colour = NA, colour = "darkgrey", family = "Arial Narrow") +
  scale_y_continuous(limits = c(690, 1080)) +
  labs(title = "LSE HR internal data") +
  personal_theme_simple_patchwork +
  theme(legend.position = "top")
```


```{r patchworkHR, fig.showtext=TRUE, fig.width=9, fig.height=6}
# patchwork plots HESA + LSE HR
plot_patchwork <- p4_patchwork + p_hr_patchwork + 
  plot_layout(guides = 'collect') +
  plot_annotation(title = "Numbers of fixed-term and permanent academic staff at LSE\naccording to HESA data and LSE HR internal data",
                  caption = "Sources: HESA 2023 (left plot); LSE HR internal data 2023 (right plot)",
...
plot_patchwork
#ggsave(plot_patchwork, filename = "plotpatchwork_HESA_LSEHR.png", width = 8, height = 5) # save plot)
```
...
tab <- tab %>%
  group_by(Staff.Type) %>%
  filter(!Staff.Type == "Policy", !Staff.Type == "LSE Fellow") %>% # remove these empty categories (there are no permanent Policy/LSE fellows)
